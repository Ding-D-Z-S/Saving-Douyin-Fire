...
async def run(dry_run: bool = False, env_file: str | None = None) -> int:
    ...
    while next_target < len(task.targets) and fatal_error is None:
        retry_auth = False
        try:
            async with open_douyin(settings) as session:
                ...
                try:
                    ...
                    if auth_attempt == 0:
                        LOGGER.warning("登录状态首次检测过期，将关闭浏览器后重试一次")
                        retry_auth = True
                    else:
                        LOGGER.error("AUTH_RETRY_EXHAUSTED 登录状态连续两次检测过期")
                        results.append(TargetResult(target="登录检查", status="failed", error=str(exc)))
                        fatal_error = AuthenticationError(f"AUTH_RETRY_EXHAUSTED：登录状态连续两次检测过期：{exc}")
                    ...
                    label = "登录检查" if isinstance(exc, RiskControlError) else "运行检查"
                    results.append(TargetResult(target=label, status="failed", error=str(exc)))
                    fatal_error = exc

                if not retry_auth and fatal_error is None:
                    chat = DouyinChat(page, timeout_ms=int(task.target_open_timeout_seconds * 1000))
                    for index in range(next_target, len(task.targets)):
                        ...
                        try:
                            ...
                            if not dry_run:
                                for message_index, message in enumerate(target.messages):
                                    ...
                                    await verify_login(page, timeout_ms=3_000, require_private_marker=False)
                            ...
                            if auth_attempt == 0:
                                LOGGER.warning("处理好友时首次检测到登录过期，将重开浏览器重试一次: %s", alias)
                                retry_auth = True
                                next_target = index
                            else:
                                LOGGER.error("AUTH_RETRY_EXHAUSTED 登录状态连续两次检测过期: %s", alias)
                                screenshot = await _screenshot(page, settings.artifacts_dir, alias)
                                if screenshot:
                                    screenshots.append(screenshot)
                                error = AuthenticationError(f"AUTH_RETRY_EXHAUSTED：登录状态连续两次检测过期：{exc}")
                                results.append(TargetResult(target=target.name, status="failed", sent=sent, error=str(error), target_alias=alias))
                                fatal_error = error
                            break
                        except RiskControlError as exc:
                            LOGGER.exception("抖音要求安全验证，按风控策略不重试: %s", alias)
                            screenshot = await _screenshot(page, settings.artifacts_dir, alias)
                            if screenshot:
                                screenshots.append(screenshot)
                            ...
                            if not task.continue_on_error:
                                break
                        if index < len(task.targets) - 1 and not dry_run and not retry_auth:
                            await asyncio.sleep(random.uniform(task.interval_min, task.interval_max))
                ...
                if settings.trace and not trace_saved:
                    try:
                        ...
                        if fatal_error is None and not retry_auth:
                            fatal_error = exc
                            results.append(TargetResult(target="运行收尾", status="failed", error=str(exc)))
        except AuthenticationError as exc:
            if auth_attempt == 0:
                LOGGER.warning("浏览器启动阶段首次检测登录过期，将重试一次")
                retry_auth = True
            else:
                LOGGER.error("AUTH_RETRY_EXHAUSTED 登录状态连续两次检测过期")
                fatal_error = AuthenticationError(f"AUTH_RETRY_EXHAUSTED：登录状态连续两次检测过期：{exc}")
                results.append(TargetResult(target="登录检查", status="failed", error=str(fatal_error)))
        except Exception as exc:
...
async def _screenshot(page, artifacts_dir: Path, label: str) -> Path | None:
    ...
    try:
        ...
        if callable(is_closed) and is_closed() is True:
            LOGGER.warning("浏览器页面已关闭，跳过失败截图")
            return None
        await page.screenshot(path=path, full_page=True)
        return path
    except Exception as exc:
        if "closed" in str(exc).lower():
            LOGGER.warning("浏览器页面已关闭，无法保存失败截图")
        else:
