...
async def capture_storage_state(settings: Settings, output_path: Path, timeout_ms: int = 180_000) -> None:
    playwright: Playwright | None = None
    browser: Browser | None = None
    context: BrowserContext | None = None
    try:
        ...
        while True:
            ...
            if await _any_visible(page, LOGIN_REQUIRED_MARKERS, timeout_ms=2_000):
                ...
                try:
                    ...
                    deadline -= 2_000
            if deadline <= 0:
                raise AuthenticationError("等待扫码登录超时，请重试")
    finally:
        if context:
            await context.close()
        if browser:
...
async def verify_login(
    ...
    *,
    require_private_marker: bool = True,
) -> None:
    if page.is_closed():
        raise AuthenticationError("抖音浏览器页面已关闭")
    if await _any_visible(page, RISK_MARKERS, timeout_ms=2_000):
        raise RiskControlError("抖音要求进行安全验证，任务已停止")
    if await _any_visible(page, LOGIN_REQUIRED_MARKERS, timeout_ms=2_000):
        raise AuthenticationError("抖音登录状态已失效")
    if require_private_marker and not await _any_visible(page, LOGIN_MARKERS, timeout_ms=timeout_ms):
