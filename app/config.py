...
DEFAULT_TASK_PATH = DEFAULT_DATA_DIR / "config.json"
DEFAULT_ENV_PATH = DEFAULT_DATA_DIR / ".env.local"
DEFAULT_STATE_PATH = DEFAULT_DATA_DIR / "storage-state.json"
...
def load_settings(env_file: str | Path | None = None) -> Settings:
    dotenv_path = _project_path(env_file) if env_file else DEFAULT_ENV_PATH
    if dotenv_path.is_file():
        load_dotenv(dotenv_path=dotenv_path, override=False)
    task_path = _project_path(os.getenv("TASK_CONFIG", str(DEFAULT_TASK_PATH)))
    artifacts_dir = _project_path(os.getenv("ARTIFACTS_DIR", str(DEFAULT_ARTIFACTS_DIR)))
    ...
    storage_state_value = _optional_env("DOUYIN_STORAGE_STATE")
    browser_path_value = _optional_env("BROWSER_PATH")
    ...
    return Settings(
        ...
        storage_state=(str(_project_path(storage_state_value)) if storage_state_value else (str(default_state) if default_state.is_file() else None)),
        cookie=_optional_env("DOUYIN_COOKIE"),
        headless=_parse_bool(os.getenv("HEADLESS", "false"), "HEADLESS"),
        browser_path=(str(_project_path(browser_path_value)) if browser_path_value else None),
